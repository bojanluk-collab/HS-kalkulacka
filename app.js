...
function renderDescription(text) {
    ...
    return `<div class="desc-wrapper">
        <div class="desc" title="${safe}" onclick="this.classList.toggle('open')">${text}</div>
...
async function loadData() {
    const n = await fetch('nominal.json?v=1.3').then(r => r.json());
    const b = await fetch('benchmark.json?v=1.3').then(r => r.json());
